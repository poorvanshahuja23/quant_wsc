...
def run_mean_reversion(df: pd.DataFrame, window: int = 20, z_thresh: float = 2.0) -> pd.DataFrame:
    """
    df requires: ['Close']
    """
    data = df.copy()
    ...
    data['position'] = data['signal'].replace(to_replace=0, method=None).ffill().fillna(0)
    data.loc[data['signal'] == 0, 'position'] = 0
